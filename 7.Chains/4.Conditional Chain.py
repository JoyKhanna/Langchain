...
prompt1 = PromptTemplate(template = 'Classify the sentiment of the following feedback text into positive or negative \n {feedback} \n {format_instruction}',
                         ...
                         partial_variables = {'format_instruction' : parser2.get_format_instructions()})    # telling the llm the expected output format in prompt
                                                                                                            # itself, not actually checking output, but telling it
classifier_chain = prompt1 | model | parser2

parsed_model = model.with_structured_output(Feedback)
classifier_chain2 = prompt1 | parsed_model                      # this one is better
